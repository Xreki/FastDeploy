...
from fastdeploy.model_executor.models.utils import (get_infer_model_path,
...
if current_platform.is_cuda() and current_platform.available():
    from fastdeploy.model_executor.ops.gpu import (
        reset_stop_value, speculate_update_input_ids_cpu)
...
from fastdeploy.model_executor.models.speculate_proposers import (
    AutogressiveProposer, DraftModelProposer, EagleProposer, HydraProposer,
...
def get_parser():
    ...
    parser.add_argument("--input_file",
                        type=str,
                        default="./data/query-answers-list.jsonl")
    ...
    parser.add_argument("--perf_repeat_times",
                        default=10,
                        type=int,
                        help="Repeat times in perf mode")
    parser.add_argument(
        "--perf_output_file",
        default="perf_result",
        type=str,
...
class Predictor:
    ...
    def __init__(self, args):
        ...
        self.use_beam_search = model_config.get("infer_model_use_beam_search",
                                                False)
        gqa_use_tensorcore = model_config.get("gqa_use_tensorcore", False)
        self.speculate_enable = model_config.get(
            "infer_model_speculate_enable", False)
        ...
        if ffn2_use_hardamard:
            ...
            hardamard_block_size = model_config.get("hardamard_block_size",
                                                    None)
            if hardamard_block_size is not None:
                paddle.set_flags(
                    {"FLAGS_hardamard_block_size": hardamard_block_size})
                logger.info(
                    f"hardamard_block_size is set to {hardamard_block_size}")
                paddle.set_flags(
                    {"FLAGS_hardamard_use_diagonal_block_matrix": 1})
                logger.info(
                    "FLAGS_hardamard_use_diagonal_block_matrix is set to True")
        ...
        if self.use_beam_search:
            ...
            assert (self.beam_width % self.beam_group_num == 0
                    ), f"beam_width must be divisible by beam_group_num. \
                  But get beam_width:{self.beam_width} and beam_group_num:{self.beam_group_num}"

        if self.speculate_enable:
            assert (
                args.speculate_method
                is not None), "Speculate Model must be set a speculate_method"
        else:
            assert (args.speculate_method is None
        ...
        if "infer_model_cache_kv_type" in model_config:
            ...
            if model_config[
                    "infer_model_cache_kv_type"] == "cachekv_quant_int8":
                args.use_cache_kv_int8 = 1
                args.use_cache_kv_int4 = 0
            elif model_config[
                    "infer_model_cache_kv_type"] == "cachekv_quant_int4":
        ...
        head_dim = model_config["hidden_size"] // model_config[
            "num_attention_heads"]
        ...
        if self.args.prefix_path:
            ...
            prefix_cache = [
                item.squeeze_(0) for item in paddle.split(
                    prefix_cache, len(prefix_cache), axis=0)
            ]
            self.prefix_cache = {}
        ...
        if self.args.device == "NPU":
            ...
            if int(os.getenv("FLAGS_npu_use_compressed_mask", "0")) > 0:
                self.attention_mask = np.tril(np.ones([128,
                                                       128])).astype("float32")
            else:
                self.attention_mask = np.tril(
                    np.ones([args.max_seq_len,
                             args.max_seq_len])).astype("float32")
            self.rope_emb = get_rope(
                rotary_dim=head_dim,
                base=rope_theta,
                position_ids=tmp_position_ids.repeat_interleave(
            ...
            with CpuGuard():
                tmp_position_ids = paddle.arange(args.max_seq_len).reshape(
                    (1, -1))
                self.rope_emb = get_rope(
                    rotary_dim=head_dim,
                    base=rope_theta,
            ...
            self.rope_emb = get_rope(
                ...
                base=rope_theta,
                position_ids=tmp_position_ids,
                artial_rotary_factor=compression_ratio,
            )
        ...
        if args.lora_num > 0:
            ...
            for i in range(model_config["num_layers"]):
                ...
                ffn1_weights_lora_A.append(
                    lora_states[f"gpt.decoder.layers.{i}.linear1.lora_A"].
                    transpose((1, 0)))
                # for ffn1
                ...
                ffn2_weights_lora_A.append(
                    lora_states[f"gpt.decoder.layers.{i}.linear2.lora_A"].
                    transpose((1, 0)))
                ffn2_weights_lora_B.append(
                    lora_states[f"gpt.decoder.layers.{i}.linear2.lora_B"].
                    transpose((1, 0)))
        ...
        if args.speculate_method == "inference_with_reference":
            ...
            self.proposer = DraftModelProposer(args,
                                               args.speculate_max_draft_tokens,
                                               args.batch_size)
            ...
            self.proposer = MTPProposer(args, args.speculate_max_draft_tokens,
                                        args.batch_size)
        else:
            self.proposer = None
    ...
    def create_predictor(self, args):
        ...
        Returns:
            返回创建好的预测器对象。
        """
        infer_model_path = get_infer_model_path(args.model_name_or_path,
                                                args.model_prefix)
        ...
        config = paddle.inference.Config(full_model_path,
                                         infer_model_path + ".pdiparams")
    ...
    def preprocess(self, dials, extra_infos=None):
        ...
        # construct inputs
        if self.args.perf_input_len > 0:
            input_ids = [[10002] * self.args.perf_input_len
        ...
        input_ids = self.convert_tokens_to_input_ids(input_ids,
                                                     num_input_tokens)
        ...
        if self.beam_width > 1:
            for i in range(bs // self.beam_width):
                ...
                for beam_id in range(self.beam_width):
                    self.used_list[i * self.beam_width +
                    ...
                    for k in range(self.args.max_dec_len):
                        ...
                        inputs["block_tables"][i * self.beam_width + beam_id,
                                               query_seq_len +
                                               k] = used_block_id
        else:
            for i in range(bs):
                for j in range(
                    (seq_len[i] + self.args.max_dec_len +
        ...
        if self.args.device == "NPU":
            inputs["attention_mask"] = ((self.attention_mask - 1) *
                                        1e6).astype(self.args.dtype)
        inputs["top_p"] = (np.array([
            self.args.top_p,
        ] * self.beam_batch_size).reshape(-1, 1).astype("float32"))
        ...
        inputs["seq_lens_decoder"] = (np.array([
            0,
        ] * self.beam_batch_size).astype("int32").reshape(-1, 1))
        inputs["step_idx"] = np.array(step_idx).astype("int64").reshape(-1, 1)
        # inputs["tgt_mask"] = self.tgt_generation_mask
        inputs["min_dec_len"] = (np.array([
            0,
        ] * self.beam_batch_size).astype("int64").reshape(-1, 1))
        inputs["max_dec_len"] = (np.array([
            0,
        ] * self.beam_batch_size).astype("int64").reshape(-1, 1))
        inputs["not_need_stop"] = np.array([True]).astype("bool")
        inputs["stop_flags"] = (np.array([
            1,
        ...
        if self.args.use_system:
            # NOTE(Zhenyu Li): just for test
            inputs["system_ids"] = paddle.full(shape=[self.args.batch_size, 1],
                                               fill_value=-1,
                                               dtype="int32")
        ...
        if self.beam_width > 1:
            ...
            inputs["beam_width"] = (np.array([self.beam_width
                                              ]).astype("int32").reshape(1, 1))
            inputs["beam_group_num"] = (np.array(
                [self.beam_group_num]).astype("int32").reshape(1, 1))
    ...
    def enf_gen_step_process(self, model_inputs, batch_size):
        ...
        if self.beam_width > 1:
            ...
            step_out_list = (self.inner_predictor.get_output_handle(
                step_out_name).copy_to_cpu().tolist())
            for i in range(batch_size):
    ...
    def predict(self, dials: list[list[dict]], extra_infos=None):
        ...
        if self.return_all_hidden_states:
            for i in range(self.model_config["num_layers"]):
                model_inputs[f"value_caches_{i}"] = self.cache_kvs[
                    f"value_caches_{i}"]
                model_inputs[f"key_caches_{i}"] = self.cache_kvs[
                    f"key_caches_{i}"]
    ...
    def compute_perf_result(self):
        ...
        self.batch_time_list = self.batch_time_list[self.args.
                                                    perf_warm_up_times:]
...
p = profiler.Profiler(scheduler=[3, 4],
...
def main():
    ...
    if args.perf_input_len > 0:
        assert (args.perf_repeat_times
                > 0), f"Repeat time must > 0, but get {args.perf_repeat_times}"
        assert (args.perf_input_len + args.max_dec_len
                <= args.max_seq_len), f"input_len + \
...
= {args.perf_input_len + args.max_dec_len} > {args.max_seq_len}"
    ...
    if beam_width > 1:
        infer_dials = [
            dials for dials in infer_dials for _ in range(beam_width)
        ]
    test_case = []
    rank = dist.get_rank()
            ...
            if rank == 0:
                if beam_width > 1:
                    ...
                    predictor.num_output_tokens += ((
                        ...
                        & (output_tensor
                           != predictor.tokenizer.cls_token_id)).sum().item())
                    outputs = predictor.postprocess(output_tensor)["result"]
                else:
                    outputs = []
                    if predictor.model_config["architectures"][
                            0] == "ErnieBotRM":
                        output_name = predictor.inner_predictor.get_output_names(
                        )[0]
