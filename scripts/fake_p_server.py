...
def get_parser(add_input_output_file: bool = True):
    ...
    parser.add_argument(
        ...
        default="False",
        type=strtobool,
        help="use benchmark or not",
    )
    parser.add_argument(
        "--fake_server_p",
        default="False",
        type=strtobool,
        help="use fake_server_p or not",
    )
    parser.add_argument(
        "--prefix_path",
        default=None,
...
class Predictor:
    ...
    def __init__(self, args, tokenizer=None, model=None, **kwargs):
        ...
        if args is None:
            ...
            config, tokenizer, model, _ = build_stream_line_model(
                ...
                fake_server_p=args.fake_server_p,
            ...
            if use_cache_kv_int8 or use_cache_kv_int4:
                cache_type = "uint8"
            else:
    ...
    def preprocess(self, dials: list[list[dict]], extra_infos=None):
        ...
        if not self.args.fake_server_p:
            if os.getenv("EP_DEBUG", "False") == "True":
                input_ids = [[50] * 4383] * self.args.batch_size
                num_input_tokens = 4383 * self.args.batch_size
            else:
            ...
            for i in range(len(dials)):
                input_ids.append(dials[i])
                num_input_tokens += len(dials[i])
            print("num_input_tokens: ", num_input_tokens)
    ...
    def predict(
        ...
        if not self.args.fake_server_p:
            ...
            if self.count >= warm_up_time:
                self.preprocess_time.append(e_time1 - s_time1)
            ...
            if self.count >= warm_up_time:
                self.forward_time.append(e_time2 - s_time2)
                self.all_time.append(e_time2 - s_time1)
                logger.info(f"preprocess_time: {self.preprocess_time[-1]}")
                logger.info(f"forward_time: {self.forward_time[-1]}")
                logger.info(f"cost all time: {self.all_time[-1]}")
                logger.info(
                    f"preprocess_cost_time avg: {sum(self.preprocess_time) / len(self.preprocess_time)}"
                )
                logger.info(
                    f"forward_cost_time avg: {sum(self.forward_time) / len(self.forward_time)}"
                )
                logger.info(
                    f"all cost_time avg: {sum(self.all_time) / len(self.all_time)}"
                )
                logger.info(
                    f"ITPS ALL(all_token_num / sum_time): {all_token / sum(self.all_time)}"
                )
            self.count += 1
...
def main():
        ...
        with open(args.input_file, "r") as fin:
            if args.fake_server_p:
                ...
                for line in lines:
                    ...
                    all_token_num += len(data["input_ids"])
                    infer_dials.append(data["input_ids"])
            else:
    ...
    if args.fake_server_p:
        ...
        logger.info(f"threshold_token_num: {threshold_token_num}")
        logger.info(f"all_token_num: {all_token_num}")
        while True:
            dial_len_sum = 0
            b_num = 0
            batch_dials = []
            ...
            # 推完了，几乎空推
            if b_num == 0:
                batch_dials.append([10002])
                dial_len_sum += 1
                b_num += 1
            if len(infer_dials) <= 0:
                logger.info("end!!!")
