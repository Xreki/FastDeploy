...
function build_and_install_ops() {
  ...
  echo -e "${BLUE}[build]${NONE} build and install efficientllm_ops..."
  ...
  if [ $? -ne 0 ]; then
    echo -e "${RED}[FAIL]${NONE} build efficientllm_ops wheel failed ${NONE}"
    exit 1
  fi
  echo -e "${BLUE}[build]${NONE} ${GREEN}build efficientllm_ops success ${NONE}"
