...
function init() {
    ...
    ${python} -m pip install --force-reinstall --pre paddlepaddle-gpu -i https://www.paddlepaddle.org.cn/packages/nightly/cu118/
    ${python} -m pip install --upgrade --force-reinstall -r requirements/gpu/cu118/requirements.txt --ignore-installed PyYAML
...
function copy_ops(){
    ...
    mkdir -p ../fastdeploy/model_executor/base && cp -r ./${OPS_TMP_DIR_BASE}/${WHEEL_BASE_NAME}/* ../fastdeploy/model_executor/base
...
function build_and_install_ops() {
  ...
  BUILDING_ARCS="[80]" ${python} setup_ops.py install --install-lib ${OPS_TMP_DIR}
  if [ "$WITH_CPU" == "true" ]; then
