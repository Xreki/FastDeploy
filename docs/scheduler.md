...
### 全局调度器
全局调度器基于 Redis 实现，各个节点根据自身 GPU 负载情况主动从 Redis 中拉取任务。具体的是根据节点自身空闲 block 数、空闲 batch 数、单次支持最大 prefill 的 token 数目进行拉取，成功拉取到任务后更新空闲 block 数和空闲 batch 数。
...
| scheduler_wait_response_timeout | float    | 否       | 0.001     | local,global | 每次等待生成token结果的时间 |
...
| scheduler_remote_write_time     | int      | 否       | 3         | global       | redis一次写入的最大耗时     |