...
class DefaultModelLoader(BaseModelLoader):
    ...
    def set_quanted_status(
        self, fd_config: FDConfig, model_class: PretrainedModel
    ) -> None:
        """set_quanted_status"""
        if (
            not fd_config.model_config.is_quantized
            and fd_config.quant_config is not None
            and fd_config.model_config.pre_quant
        ):
            if (
                hasattr(model_class, "_get_quantization_mappings")
                and model_class._get_quantization_mappings
            ):
    ...
    def load_model(self, fd_config: FDConfig) -> nn.Layer:
        ...
        self.set_quanted_status(fd_config, model_class)
        with context:
        ...
        state_dict = load_composite_checkpoint(
            ...
            model_class,
            fd_config,
            dict(model.named_sublayers()),
            return_numpy=True,
        )
        model.set_state_dict(state_dict)
        self.clean_memory_fragments(state_dict, model)
        return model
