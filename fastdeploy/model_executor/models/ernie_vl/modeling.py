...
from fastdeploy.config import PDConfig
...
class Ernie45TVLMoE(nn.Layer):
    ...
    def __init__(self, pd_config: PDConfig, layer_id: int,
        ...
        self.fused_moe_text = FusedMoE(
            pd_config=pd_config,
            moe_intermediate_size=pd_config.moe_config.
            moe_intermediate_size[0],
            num_experts=pd_config.moe_config.num_experts[0],
        ...
        self.fused_moe_image = FusedMoE(
            ...
            num_experts=pd_config.moe_config.num_experts[1],
            top_k=pd_config.moe_config.top_k,
            moe_use_gate_correction_bias=pd_config.moe_config.
            moe_use_gate_correction_bias,
            moe_quant_type=pd_config.moe_config.moe_quant_type,
...
class Ernie45TVLDecoderLayer(nn.Layer):
    ...
    def __init__(
        ...
        self.input_layernorm = RMSNorm(
            pd_config,
            hidden_size=pd_config.model_config.hidden_size,
            eps=1e-5,
...
class Ernie45TVLModel(nn.Layer):
    ...
    def __init__(
        ...
        self.num_layers = pd_config.model_config.num_layers
        self.im_patch_id = pd_config.moe_config.im_patch_id
        self._dtype = pd_config.model_config.dtype
        pd_config.model_config.prefix_name = "ernie"

        self.embeddings = VocabParallelEmbedding(
            pd_config=pd_config,
            num_embeddings=pd_config.model_config.vocab_size,
            embedding_dim=pd_config.model_config.hidden_size,
            params_dtype=paddle.get_default_dtype,
            prefix=(f"{pd_config.model_config.prefix_name}.embed_tokens"),
        )

        self.hidden_layers = [
            Ernie45TVLDecoderLayer(
                pd_config=pd_config,
                prefix=f"{pd_config.model_config.prefix_name}.layers.{i}")
        ...
        self.norm = RMSNorm(
            ...
            hidden_size=pd_config.model_config.hidden_size,
            eps=1e-5,
            prefix=f"{pd_config.model_config.prefix_name}.norm",
        )
...
class ErnieMoEVLForCausalLM(ModelForCasualLM):
    ...
    def __init__(self, pd_config: PDConfig):
        """
        Args:
            pd_config (PDConfig): Configurations for the LLM model.
        """
        super(ErnieMoEVLForCausalLM, self).__init__(pd_config)

        self.model = Ernie45TVLModel(pd_config=pd_config)

        self.ori_vocab_size = pd_config.model_config.ori_vocab_size
