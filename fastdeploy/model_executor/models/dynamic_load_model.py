...
class DynamicLoadModel(nn.Layer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        logger.info("EfficientLLM model built successfully by DynamicLoadModel")
    ...
    def _build_model(self) -> paddle.nn.Layer:
        ...
        _, _, model = build_stream_line_model(
            ...
            embeddings_column_cut=self.kwargs.get("embeddings_column_cut", False),
    ...
    def verify_parameters_updated(self, erro_log: bool = True) -> bool:
        ...
        for name, param in self.model.state_dict().items():
            if not param._is_initialized():
                if erro_log:
                    logger.error(f"Parameter {name}-{param} was not properly cleared!")
                all_update = False
