    ...
    for layer_id in range(num_layers):
        for export_id in range(num_experts):
            for key, action in base_actions.items():
                ...
                if (
                    LayerIdPlaceholder.MOE_LAYER_ID in placeholders
                    and layer_id < start_layer
                ):
                    continue
                if (
                    LayerIdPlaceholder.FFN_LAYER_ID in placeholders
                    and layer_id >= start_layer
                ):
                    continue
                ...
                if LayerIdPlaceholder.LAYER_ID.value in placeholders:
                    params[LayerIdPlaceholder.LAYER_ID.value] = layer_id
                if LayerIdPlaceholder.FFN_LAYER_ID.value in placeholders:
                    params[LayerIdPlaceholder.FFN_LAYER_ID.value] = layer_id
                if LayerIdPlaceholder.MOE_LAYER_ID.value in placeholders:
                    params[LayerIdPlaceholder.MOE_LAYER_ID.value] = layer_id
                if LayerIdPlaceholder.EXPERT_ID.value in placeholders:
                    params[LayerIdPlaceholder.EXPERT_ID.value] = export_id
