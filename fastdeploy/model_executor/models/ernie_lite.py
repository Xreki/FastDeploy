...
class Ernie45LiteAttention(nn.Layer):
...
class Ernie45LiteDecoderLayer(nn.Layer):
    ...
    def __init__(
        ...
        self.self_attn = Ernie45LiteAttention(
            llm_config=llm_config,
            layer_id=layer_id,
        ...
        if (llm_config.moe_config.num_experts is not None
            ...
            self.mlp = FusedMoE(
                ...
                shared_experts_up_gate_proj_key=
                f"{prefix}.mlp.shared_experts.up_gate_proj",
                shared_experts_down_proj_key=
                f"{prefix}.mlp.shared_experts.down_proj",
            )
        else:
            self.mlp = Ernie45LiteMLP(
                llm_config=llm_config,
                prefix=f"{prefix}.mlp",
            )
...
class Ernie45LiteModel(nn.Layer):
    ...
    def __init__(
        ...
        """
        Initializer for the Ernie45LiteModel class.
...
class ErnieForCausalLM(ModelForCasualLM):
    ...
    def __init__(self, llm_config: LLMConfig):
        ...
        self.model = Ernie45LiteModel(llm_config=llm_config)
