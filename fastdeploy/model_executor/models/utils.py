...
def get_rotary_position_embedding(position_ids,
                                  head_dim,
                                  compression_ratio=1,
                                  rope_theta=10000.0):
    """
    ...
    inv_freq = rope_theta**(-paddle.arange(0, head_dim, 2, dtype="float32") /
                            head_dim)
    compression_position_ids = position_ids / compression_ratio

    # shape: [B, S, D/2]
    freqs = paddle.einsum("ij,k->ijk",
                          compression_position_ids.cast("float32"), inv_freq)
    ...
    if paddle.is_compiled_with_xpu():
        ...
        # shape: [B, S, D/2]
        rot_emb = paddle.zeros((2, bsz, max_seq_len, 1, head_dim // 2),
                               dtype="float32")
    ...
    if paddle.is_compiled_with_custom_device("npu"):
        return (paddle.concat([rot_emb, rot_emb], axis=3).transpose(
            [0, 1, 2, 4, 3]).reshape([2, bsz, max_seq_len, 1, head_dim]))
    else:
        return rot_emb
...
def get_rotary_position_embedding_3d(
    ...
    position_ids_3d = paddle.tile(
        paddle.arange(seq_len, dtype="int64").unsqueeze(0).unsqueeze(-1),
        [1, 1, 3])
    ...
    position_ids = paddle.arange(0, seq_len, 1, dtype="float32").reshape(
        (1, -1))
