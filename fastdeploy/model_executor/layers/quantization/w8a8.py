...
class W8A8LinearMethod(QuantMethodBase):
    ...
    def create_weights(self, layer):
        ...
        # (tangbinhan todo) Skip layer quantization is not supported now.
    ...
    def apply_weight_quantization(self, weights):
        ...
        weight_tensor = paddle.cast(
            weight_tensor, self.quant_config.weight_dtype)
        return weight_tensor, None

    def process_unquantized_weights(self, layer, weights) -> None:
        """process_unquantized_weights"""
        if self.quant_config.use_smooth_quant:
            self.smooth_quant_method.process_unquantized_weights(layer, weights)
        if self.skip_quant:
