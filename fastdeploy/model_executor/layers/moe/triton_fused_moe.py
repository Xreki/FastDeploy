...
class TritonFusedMoeMethod(FusedMoEMethodBase):
    ...
    def create_weights(
        ...
        assert moe_quant_type == "weight_only_int8"
        ...
        if moe_quant_type == "weight_only_int8":
            max_bound = 127
        elif moe_quant_type == "weight_only_int4":
            max_bound = 7
    ...
    def apply(
        ...
        """
        token_num = x.shape[0]
        top_k = moe_compute_params.top_k
        num_local_experts = moe_compute_params.num_local_experts
        top_k = moe_compute_params.top_k
        moe_intermediate_size = moe_compute_params.moe_intermediate_size
        hidden_size = moe_compute_params.hidden_size
        ...
        sorted_token_ids, expert_ids, num_tokens_post_padded = tritonmoe_preprocess(topk_ids, num_local_experts, config["BLOCK_SIZE_M"])
        max_num_tokens_padded = sorted_token_ids.shape[0]
        grid = (ceil_div(max_num_tokens_padded,config["BLOCK_SIZE_M"]) * ceil_div(moe_intermediate_size*2,  config["BLOCK_SIZE_N"]),)
