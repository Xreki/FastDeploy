...
class IPCCommManager:
    ...
    def write_cache(self, remote_gpu_id, local_block_ids, remote_block_ids,
                    layer_idx):
        """
...
class IPCCacheTransfer(object):
    ...
    def __init__(self,
        ...
        for layer_idx in range(self.num_layers):
            cache_k.append(self.gpu_cache_kvs[
                f'key_caches_{layer_idx}_rank{self.rank}_device{gpu_id}'])
            cache_v.append(self.gpu_cache_kvs[
        ...
        logger.info(
            f"creating ipc_comm with local_device_id:{local_device_id}, ")
        ...
        try:
            ...
            logger.info("start prefill layerwise send cache thread")

        except Exception as e:
            logger.info(f"create ipc_comm failed, {e}")
            raise e

        logger.info("cache messager init finished, use ipc")

    def _prefill_layerwise_send_cache_thread(self):
        """
        prefill layerwise send cache thread
        """
        logger.info("prefill layerwise send cache thread start")
        try:
            ...
            while True:
                ...
                if not cache_info:
                    ...
                    for info in cache_info:
                        if info['request_id'] in self.cache_info:
                            ...
                            src_block_ids = current_info["src_block_ids"][
                ...
                logger.info(
                    f"current_step_idx:{step_shm_value.value[0]} {self.last_step_idx}"
                )
                logger.info(
                    f"current_layer_idx:{layer_shm_value.value[0]} {self.last_layer_idx}"
                )
                ...
                while 1:
                    ...
                    for item in current_cache_info:
                        for layer_idx in range(self.last_layer_idx + 1,
                                               prefilled_layer_idx + 1):
                            target_gpu_id = int(item['device_ids'][self.rank])
                            src_block_ids = paddle.to_tensor(
                                item['src_block_ids'],
                                dtype='int32',
                                place='cpu')
                            dest_block_ids = paddle.to_tensor(
                                item['dest_block_ids'],
                                dtype='int32',
                                place='cpu')
                            self.messager.write_cache(target_gpu_id,
                                                      src_block_ids,
                                                      dest_block_ids,
                                                      layer_idx)
                        cache_sent_set.add((int(item['device_ids'][self.rank]),
                                            item['request_id']))
                ...
                if (self.last_layer_idx + 1) == self.num_layers:
                    logger.info(
                        f"num of finish write cache task: {len(cache_sent_set)}"
                    )
