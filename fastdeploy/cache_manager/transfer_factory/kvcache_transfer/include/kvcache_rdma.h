...
public:
  ...
  RDMACommunicator(std::string &role, int gpu_idx, std::string &port,
                   ...
                   std::vector<int64_t> local_value_cache, int block_number,
                   int block_bytes);
  ~RDMACommunicator();

  // Connection management
  ConnStatus connect(const std::string &dst_ip, const std::string &dst_port);
...
private:
  ...
  bool post_block_send(struct RdmaContext *ctx, int layer_idx,
                       ...
                       std::vector<uint64_t> &remote_addr, uint32_t rkey,
