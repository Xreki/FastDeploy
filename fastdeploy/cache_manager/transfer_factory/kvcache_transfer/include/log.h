...
// Constants for log levels (C++ enum class style in C)
typedef enum {
  KV_LOG_LEVEL_INFO = 0,
...
#define GET_CURRENT_TIME()                                                     \
  do {                                                                         \
    ...
    sprintf(str, "%02d:%02d:%02d][%.32s][%d", t->tm_hour, t->tm_min,           \
            t->tm_sec, hostname, gettid());                                    \
  } while (0)

#define LOGE(fmt, arg...)                                                      \
  do {                                                                         \
    GET_CURRENT_TIME();                                                        \
    fprintf(stderr, "[%s][ERR][KV_CACHE][%s:%d] " fmt "\n", str,               \
...
#define LOGW(fmt, arg...)                                                      \
  do {                                                                         \
    GET_CURRENT_TIME();                                                        \
    fprintf(stderr, "[%s][WARN][KV_CACHE][%s:%d] " fmt "\n", str,              \
            FILE_NAME(__FILE__), __LINE__, ##arg);                             \
  } while (0)
...
#define LOGI(fmt, arg...)                                                      \
  do {                                                                         \
    GET_CURRENT_TIME();                                                        \
    fprintf(stdout, "[%s][INFO][KV_CACHE][%s:%d] " fmt "\n", str,              \
            FILE_NAME(__FILE__), __LINE__, ##arg);                             \
  } while (0)
