...
RDMACommunicator::RDMACommunicator(std::string &role, int gpu_idx,
    ...
    : splitwise_role(role), gpu_idx(gpu_idx), port(port),
      local_cache_key_ptr_layer_head_(std::move(local_key_cache)),
      local_cache_value_ptr_layer_head_(std::move(local_value_cache)),
      block_number(block_number), block_size_byte(block_bytes),
      RDMACommunicator_status(0), rdma_event_channel_epoll_fd(-1) {

  try {
    WARN("Initializing RDMA communicator for role: %s", role.c_str());

    // Step 1: Initialize KV cache structure
    ...
    // This will be used for RDMA connection establishment
    main_ip_list = get_net_ifname();
    validate_addr();
...
int RDMACommunicator::start_server(int sport, int sgid_idx, int gpu_index) {
  ...
  if (g_ib_all_devs.size() == 0) {
    if (parse_port_ib_info() != 0) {
      ERR("decode parse_port_ib_info error");
      return -1;
    }
  }
  ...
  while (RDMACommunicator_status == 1) {
    ...
    for (int i = 0; i < nfds; i++) {
      ...
      if (event_fd == sockfd) {
        ...
        if (sgid_idx >= 0) {
          if (ibv_query_gid(ctx->context, ib_dev->port, sgid_idx,
                            &local_dest->gid)) {
            close_server_connection(connfd, ctx, epollfd, connectionContexts);
        ...
        if (modify_qp_to_rts(ctx, ib_dev->port, local_dest->psn, rem_dest.get(),
                             sgid_idx) != QpStatus::kSuccess) {
          close_server_connection(connfd, ctx, epollfd, connectionContexts);
...
void RDMACommunicator::close_server_connection(
    int fd, struct RdmaContext *ctx, int epollfd,
    std::map<int, struct RdmaContext *> &connectionContexts) {
  if (ctx) {
...
void RDMACommunicator::close_client_connection(int fd, struct RdmaContext *ctx,
                                               int epollfd) {
  if (!ctx) {
  ...
  for (size_t i = 0; i < ctx->conn.read_bufs.size(); ++i) {
    if (ctx->conn.read_mrs[i])
      ibv_dereg_mr(ctx->conn.read_mrs[i]);
...
int RDMACommunicator::init_server() {
  ...
  return start_server(get_rdma_dest_port(port), get_rdma_gid_index(), gpu_idx);
}
...
ConnStatus RDMACommunicator::connect(const std::string &dst_ip,
  ...
  if (g_ib_all_devs.size() == 0) {
    if (parse_port_ib_info() != 0) {
      ERR("prefill parse_port_ib_info is error");
      return ConnStatus::kInvalidParameters;
    }
  }
  ...
  if (!client_exchange_destinations(ctx, ib_dev->port,
                                    get_rdma_dest_port(dst_port),
                                    get_rdma_gid_index(), dst_ip)) {
  ...
  for (size_t i = 0; i < block_number; ++i) {
    ...
    ctx->conn.read_mrs[i] = ibv_reg_mr(ctx->pd, ctx->conn.read_bufs[i],
                                       block_size_byte, IBV_ACCESS_LOCAL_WRITE);
    if (!ctx->conn.read_mrs[i]) {
  ...
  if (ctx->sock_fd != 0) {
    ...
    ev.data.ptr = ctx;
    int ret = epoll_ctl(rdma_event_channel_epoll_fd, EPOLL_CTL_ADD,
                        ctx->sock_fd, &ev);
    if (ret != 0) {
  ...
  while (RDMACommunicator_status == 1) {
    ...
    for (int i = 0; i < nfds; ++i) {
      ...
      if (events[i].events & (EPOLLERR | EPOLLHUP | EPOLLRDHUP)) {
        ...
        if (err)
          ERR("Socket error: %s", strerror(err));
      ...
      if (events[i].events & EPOLLIN) {
        ...
        if (bytes_read <= 0) {
          ...
          std::lock_guard<std::mutex> lock(mutex_);
          close_client_connection(ctx->sock_fd, ctx,
                                  rdma_event_channel_epoll_fd);
        }
      }
    }
  }
 ...
 */
struct ibv_mr *RDMACommunicator::register_memory_region(ibv_pd *pd, void *addr,
                                                        size_t size,
  ...
  if (getenv("KVCACHE_RELAX_ORDERING")) {
...
bool RDMACommunicator::client_mr_register_per_layer(RdmaContext *ctx) {
  ...
  const uint32_t access_flags =
      ...
      (getenv("KVCACHE_RELAX_ORDERING") ? IBV_ACCESS_RELAXED_ORDERING : 0);

  for (int i = 0; i < static_cast<int>(list_size); ++i) {
    void *key_ptr =
        reinterpret_cast<void *>(local_cache_key_ptr_layer_head_[i]);
    void *val_ptr =
        reinterpret_cast<void *>(local_cache_value_ptr_layer_head_[i]);
    size_t size = static_cast<size_t>(block_size_byte) * block_number;

    write_mr_key_list[i] =
        register_memory_region(ctx->pd, key_ptr, size,
                               "client_key_" + std::to_string(i), access_flags);
    if (!write_mr_key_list[i])
      goto fail;

    write_mr_value_list[i] = register_memory_region(
        ctx->pd, val_ptr, size, "client_value_" + std::to_string(i),
        access_flags);
    if (!write_mr_value_list[i])
      goto fail;
  }
...
int RDMACommunicator::write_cache(const std::string &ip,
  ...
  const bool debug_mode = (std::getenv("KVCACHE_DEBUG") != nullptr);
  if (debug_mode) {
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    if (!post_block_send(ctx, layer_idx, local_block_ids, is_key, remote_addr,
                         rkey, ip, port)) {
      return -1;
    }
  }

  if (std::getenv("KVCACHE_DEBUG")) {
    auto duration_us = std::chrono::duration_cast<std::chrono::microseconds>(
                           std::chrono::steady_clock::now() - start_time)
                           .count();

    DEBUG("Write cache completed - IP: %s, Port: %s, Layer: %d, BlockSize: %d, "
          "Blocks: %lu, Duration: %ld us",
          ip.c_str(), port.c_str(), layer_idx, block_size_byte, block_num,
          duration_us);
  }
  return 0;
}
...
bool RDMACommunicator::post_block_send(
    struct RdmaContext *ctx, int layer_idx,
    const std::vector<int64_t> &local_block_ids, bool is_key,
    std::vector<uint64_t> &remote_addr, uint32_t rkey, const std::string &ip,
    const std::string &port) {
  auto block_num = local_block_ids.size();
  assert(block_num > 0 && "block_num must be > 1");
    ...
    sge_list[scnt].addr =
        (uintptr_t)(is_key
                        ? local_cache_key_ptr_per_layer[layer_idx]
                                                       [local_block_ids[scnt]]
                        : local_cache_value_ptr_per_layer
                              [layer_idx][local_block_ids[scnt]]);
    sge_list[scnt].length = block_size_byte;
    sge_list[scnt].lkey = (is_key ? write_mr_key_list[layer_idx]->lkey
                                  : write_mr_value_list[layer_idx]->lkey);

    send_wr_list[scnt].wr_id = scnt;
    send_wr_list[scnt].next =
        (idx == RDMA_WR_LIST_MAX_SIZE - 1 || scnt == block_num - 1
             ? NULL
             : &send_wr_list[scnt + 1]);
    send_wr_list[scnt].sg_list = &sge_list[scnt];
    send_wr_list[scnt].num_sge = 1;
    send_wr_list[scnt].opcode = IBV_WR_RDMA_WRITE;
    send_wr_list[scnt].send_flags =
        (scnt == block_num - 1 ? IBV_SEND_SIGNALED : 0);
    send_wr_list[scnt].wr.rdma.remote_addr = remote_addr[scnt];
    send_wr_list[scnt].wr.rdma.rkey = rkey;

    inflight_wr++;

    if (idx == RDMA_WR_LIST_MAX_SIZE - 1 || scnt == block_num - 1) {
      do {
        if (inflight_wr >= RDMA_SQ_MAX_SIZE) {
          send_wr_list[scnt].send_flags = IBV_SEND_SIGNALED;
        }
        ret = ibv_post_send(ctx->qp, &send_wr_list[scnt - idx], &bad_wr);
        if (ret == 0) {
          if (inflight_wr >= RDMA_SQ_MAX_SIZE || scnt == block_num - 1) {
            ctx->conn.wc_count = 0;
            ctx->conn.wc_target_count = 0;
            if (!poll_cq_with_timeout(ctx, RDMA_POLL_CQE_TIMEOUT, 1)) {
              ERR("Polling CQ failed with error: %s (errno: %d)",
          ...
          ERR("ibv_post_send failed with error: %s (errno: %d), retrying %d/%d",
      ...
      if (ret != 0) {
        ERR("ibv_post_send failed after %d retries with error: %s (errno: %d)",
  ...
  if (success) {
    ...
    ibv_sge read_sge = {
        .addr = reinterpret_cast<uintptr_t>(ctx->conn.read_bufs[last_idx]),
        .length = static_cast<uint32_t>(block_size_byte),
        .lkey = ctx->conn.read_mrs[last_idx]->lkey};

    ibv_send_wr read_wr = {};
    read_wr.wr_id = block_num + last_idx;
    read_wr.sg_list = &read_sge;
    read_wr.num_sge = 1;
    read_wr.opcode = IBV_WR_RDMA_READ;
    read_wr.send_flags = IBV_SEND_SIGNALED;
    read_wr.wr.rdma.remote_addr = remote_addr[last_idx];
    read_wr.wr.rdma.rkey = rkey;

    ibv_send_wr *bad_rd_wr = nullptr;
    ret = ibv_post_send(ctx->qp, &read_wr, &bad_rd_wr);
    if (ret != 0) {
      ERR("RDMA Read ibv_post_send failed: %s (errno: %d)", strerror(errno),
          errno);
      success = false;
    } else {
      if (!poll_cq_with_timeout(ctx, RDMA_POLL_CQE_TIMEOUT, 1)) {
        ERR("Polling CQ failed after RDMA Read");
        success = false;
      }
    }

    if (success) {
      const char *check_env = std::getenv("KVCACHE_CHECK_SUM");
      if (check_env && std::strcmp(check_env, "1") == 0) {
        uint8_t *data =
            reinterpret_cast<uint8_t *>(ctx->conn.read_bufs[last_idx]);
        uint8_t last_byte = data[block_size_byte - 1];
        DEBUG(
            "Received data at last block %zu (Layer: %d, IP: %s, Port: %s): %u",
            last_idx, layer_idx, ip.c_str(), port.c_str(),
