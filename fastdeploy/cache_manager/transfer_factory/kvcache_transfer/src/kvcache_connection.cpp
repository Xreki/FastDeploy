 ...
 */
void busid_to_int64(const char *busId, int64_t *id) {
  char hexStr[17] = {0};
  int hexOffset = 0;
...
int parse_port_ib_info() {
  ...
  const char *env_nics = getenv("KVCACHE_RDMA_NICS");
  if (!env_nics) {
  ...
  for (int i = 0;
       i < total_devs && g_kvcache_ib_dev_nums < KVCACHE_RDMA_MAX_NICS; ++i) {
    ...
    for (int port_num = 1; port_num <= dev_attr.phys_port_cnt; ++port_num) {
      ...
      if (ibv_query_port(ctx, port_num, &port_attr) != 0) {
        WARN("Failed to query port %d on device %s: %s", port_num, dev_name,
             strerror(errno));
        continue;
      }

      if (port_attr.state != IBV_PORT_ACTIVE) {
        WARN("Port %d on device %s is not active (state: %d)", port_num,
             dev_name, port_attr.state);
        continue;
      }
      ...
      if (port_attr.link_layer != IBV_LINK_LAYER_INFINIBAND &&
        ...
        WARN("Unsupported link layer %d on device %s port %d",
             port_attr.link_layer, dev_name, port_num);
        continue;
      }
...
int get_rdma_gid_index() {
  const char *gid_index_str = std::getenv("KVCACHE_RDMA_GID_INDEX");
  int gid_index = 3;

  if (gid_index_str != nullptr) {
    try {
      gid_index = std::stoi(gid_index_str);
    } catch (const std::invalid_argument &e) {
      ERR("Invalid value for KVCACHE_RDMA_GID_INDEX, using default: %d",
          gid_index);
    } catch (const std::out_of_range &e) {
      ERR("KVCACHE_RDMA_GID_INDEX value out of range, using default: %d",
          gid_index);
    }
  }
  INFO("Using KVCACHE_RDMA_GID_INDEX: %d", gid_index);

  return gid_index;
}

int get_rdma_dest_port(std::string port) {
  const char *dest_port_str = std::getenv("SET_RDMA_DEST_PORT");
  int dest_port = std::stoi(port);

  if (dest_port_str != nullptr) {
    try {
      dest_port = std::stoi(dest_port_str);
    } catch (const std::invalid_argument &e) {
      ERR("Invalid value for SET_RDMA_DEST_PORT, using default: %d", dest_port);
    } catch (const std::out_of_range &e) {
      ERR("SET_RDMA_DEST_PORT value out of range, using default: %d",
          dest_port);
    }
  }
  INFO("Using SET_RDMA_DEST_PORT: %d", dest_port);
...
int server_exchange_qp_info(int connfd, QpInfo *local_dest, QpInfo *rem_dest) {
  ...
  if (n != static_cast<int>(QpInfo::size)) {
    ERR("Failed to read remote QP info: read %d bytes, expected %zu", n,
        QpInfo::size);
    return -1;
  }
...
QpStatus modify_qp_to_rts(struct RdmaContext *ctx, int port, int my_psn,
  ...
  if (ibv_query_device(ctx->context, &dev_attr)) {
    ERR("Failed to query device attributes: %s (errno=%d)", strerror(errno),
        errno);
  ...
  if (ibv_query_port(ctx->context, port, &port_attr)) {
    ERR("Failed to query port attributes: %s (errno=%d)", strerror(errno),
        errno);
    return QpStatus::kPortQueryFailed;
  }

  if (dest->mtu > port_attr.active_mtu) {
    ERR("Specified MTU (%d) is greater than active port MTU (%d)", dest->mtu,
  ...
  const char *env_timeout = getenv("KVCACHE_IB_TIMEOUT");
  int qp_timeout = (env_timeout != NULL) ? atoi(env_timeout) : 18;
...
static QpInfo *client_exch_dest(struct RdmaContext *ctx,
                                const std::string &dst_ip, int port,
  ...
  if (ret != 0) {
    ERR("getaddrinfo failed for %s:%d - %s", dst_ip.c_str(), port,
        gai_strerror(ret));
    return nullptr;
  }
...
bool poll_cq_with_timeout(struct RdmaContext *ctx, int timeout_seconds,
                          int cqe_count) {
    ...
    if (poll_result < 0) {
      ...
      for (int i = 0; i < poll_result; ++i) {
        if (wc_array[i].status == IBV_WC_SUCCESS) {
          ...
          LOGD("Work completion %d status is %d (%s)", poll_result,
               wc_array[i].status, ibv_wc_status_str(wc_array[i].status));
        }
      }
      free(wc_array);
      return true;
...
struct RdmaContext *create_qp(struct IbDeviceInfo *ib_dev,
  ...
  memset(ctx, 0, sizeof(struct RdmaContext)); // 初始化内存
  ...
  INFO("Successfully created QP 0x%x on device %s", ctx->qp->qp_num,
       ib_dev->devName);
 ...
 */
bool client_exchange_destinations(struct RdmaContext *ctx, int ib_port,
                                  unsigned int port, int gidx,
  ...
  LOGD("Remote address - LID: 0x%04x, QPN: 0x%06x, PSN: 0x%06x, Mtu: %u",
       rem_dest.lid, rem_dest.qpn, rem_dest.psn, temp_rem_dest->mtu);

  // Modify QP to RTS state
  if (modify_qp_to_rts(ctx, ib_port, my_dest.psn, &rem_dest, gidx) !=
      QpStatus::kSuccess) {
 ...
 */
template <typename T>
bool exchange_mr_vector(struct RdmaContext *ctx, std::vector<T> &data_list,
                        bool is_client) {
  if (is_client) {
    return client_receive_memory_region(ctx, data_list.data(),
                                        data_list.size() * sizeof(T));
  } else {
    return server_send_memory_region(ctx, data_list.data(),
                                     data_list.size() * sizeof(T));
  }
}
...
bool client_exchange_mr(struct RdmaContext *ctx) {
  ...
  if (!exchange_mr_vector(ctx, key_ptrs, true))
    return false;
  if (!exchange_mr_vector(ctx, key_rkeys, true))
    return false;
  if (!exchange_mr_vector(ctx, val_ptrs, true))
    return false;
  if (!exchange_mr_vector(ctx, val_rkeys, true))
    return false;
...
bool server_exchange_mr(struct RdmaContext *ctx) {
  ...
  if (!exchange_mr_vector(ctx, send_key_ptrs, false))
    return false;
  if (!exchange_mr_vector(ctx, send_key_rkeys, false))
    return false;
  if (!exchange_mr_vector(ctx, send_val_ptrs, false))
    return false;
  if (!exchange_mr_vector(ctx, send_val_rkeys, false))
    return false;
 ...
 * @return true on success, false on failure
 */
bool server_send_memory_region(struct RdmaContext *ctx, void *local_mr,
                               int byte_num) {
 ...
 * @return true on success, false on failure
 */
bool client_receive_memory_region(struct RdmaContext *ctx, void *remote_mr,
                                  int byte_num) {
...
int setup_listening_socket(int port) {
  ...
  const char *ifname = std::getenv("KVCACHE_SOCKET_IFNAME");
...
std::vector<std::string> get_net_ifname() {
  ...
  const char *tmp = std::getenv("KV_CACHE_SOCKET_IFNAME");
  if (tmp) {
