...
from fastdeploy.cache_manager.transfer_factory import RDMACommManager
...
class CacheMessager(object):
    ...
    """
    def __init__(self, splitwise_role, transfer_protocol, engine_worker_queue_port,
        gpu_cache_kvs, rank, nranks, num_layers,  gpu_id=0, rdma_port=None):
        """
        ...
        if key_cache.dtype == paddle.bfloat16:
            block_bytes *= 2
        logger.info(f"layers {num_layers} cache_shape: {cache_shape}, max_block_num: {max_block_num}, "
        ...
        for protocol in transfer_protocol:
            if protocol == "ipc":
                ...
                self.messager[protocol] = RDMACommManager(splitwise_role,
                                                            rank,
                                                            gpu_id,
                                                            cache_k_ptr_list,
                                                            cache_v_ptr_list,
            ...
            while True:
                    ...
                    for info in cache_info:
                        if info['request_id'] in self.cache_info:
                            ...
                            if "dest_block_ids" in current_info and "src_block_ids" in current_info:
                                current_src_blocks = current_info["src_block_ids"][:len(current_info["dest_block_ids"])]
                ...
                for req_id, item in list(self.cache_info.items()):
                    ...
                    if item["transfer_protocol"] == "rdma":
                        target_ip = item['rdma_ip'][self.rank]
                    ...
                    for layer_idx in range(item["layer_idx"], current_layer_idx):
                        ...
                        if return_code != 0:
                            ...
                            logger.error(f"write cache failed, layer_idx: {layer_idx}, "
                                f"req_id: {item['request_id']}, dest_ip: {target_ip}")
                            break
                        if current_transfer_protocol == "rdma":
                            self.messager["rdma"].is_write_completed(target_ip, target_id, src_block_ids) 
                        tok = time.time()
                        cost_time = tok - tic
                        ...
                        logger.debug(f"finish write cache for a layer, {item['request_id']}, {layer_idx}"
