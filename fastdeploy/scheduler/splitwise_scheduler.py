...
class SplitWiseScheduler(object):
    ...
    def get_requests(self, available_blocks, block_size, reserved_output_blocks,
                    max_num_batched_tokens, batch=1):
        """
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                logger.error(f"ResultsReader{self.idx} sync results error: {str(e)}")
...
class APIScheduler(object):
    ...
    def __init__(self, config):
        ...
        self.client = redis.Redis(host=config.redis_host, port=config.redis_port, password=config.redis_password)
    ...
    def start(self):
        ...
        for i in range(self.reader_parallel):
            reader = ResultReader(self.client, i, self.reader_batch_size, self.ttl)
        ...
        while True:
            try:
                ...
                if len(mnodes) == 0 and (len(pnodes) == 0 or len(dnodes) == 0):
                    logger.error(f"No Schedule Nodes: mixed:{len(mnodes)}, prefill:{len(pnodes)}, decode:{len(dnodes)}")
                    time.sleep(1)
                    continue
