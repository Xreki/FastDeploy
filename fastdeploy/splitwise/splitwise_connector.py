...
from fastdeploy.engine.request import RequestOutput, CompletionOutput
...
class SplitwiseConnector:
            ...
            while True:
                try:
                    ...
                    if not socks:
                        ...
                        logger.info(f"receive {socks}")
                    

                    sender_id, _, message = self.router_socket.recv_multipart()
    ...
    def send_first_token(self, prefill_msg, tasks_list):
        ...
        logger.info(f"send first token to port decode")
        ...
        for i in range(len(tasks)):
            ...
            if tasks[i].disaggregate_info["role"] == "decode":
                if tasks[i].disaggregate_info["transfer_protocol"] == "ipc":
                    ...
                    cache_info = {"request_id": tasks[i].request_id,
                            "device_ids": self.cfg.device_ids.split(","),
                            "rdma_ip": self.cfg.disaggregate_info["cache_info"]["rdma"]["rdma_ip"],
    ...
    def _handle_prefill(self, tasks):
        ...
        self.engine_worker_queue.put_splitwise_tasks(("decode", tasks_data))
    ...
    def _handle_decode(self, payload):
        ...
        self.engine_worker_queue.put_splitwise_tasks(("decode", tasks))
