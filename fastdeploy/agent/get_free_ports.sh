function get_free_port() {
  ...
  free_ports=()
  if [[ ! -n ${free_ports_num} || "${free_ports_num}" -le 0 || "${free_ports_num}" -gt 20 ]]; then
    log_warn "param can't be empty, and should > 0 and <= 20"
    echo ${free_ports[@]}
    return 1
  fi
...
free_ports=($(get_free_port $1 8000 9000))
