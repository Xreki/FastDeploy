...
class LlamaProcessor(BaseDataProcessor):
    """
    初始化模型实例。
    ...
    Attributes:
        ...
        pad_token_id (int): 存储填充符号的token ID。
    """
    def __init__(self, model_name_or_path,max_batch_size=8):
    ...
    def text2ids(self, text, history_qa=None, max_seq_len=None, system=None):
        ...
        if isinstance(text, list):
            ...
            messages.append(text)
        tokens,input_ids = self._convert_to_ids(messages, max_seq_len, system)
        data_processor_logger.debug(f"processed data : {''.join(tokens)}")
        return input_ids
    ...
    def _convert_to_ids(self, messages, max_seq_len=None, system=None):
        ...
        Args:
            messages (List[str]): 包含所有对话轮的文本列表。
            max_seq_len (int, optional): 最大序列长度。
            system (str, optional): 系统设定。
        ...
        if self.tokenizer.chat_template is not None:
            # 如果tokenizer有聊天模板
            if not isinstance(messages, list) or not isinstance(messages[0], str):
                messages = [messages]
                # 如果messages不是列表或列表中的元素不是字符串
                messages = [messages]
            # 将messages转换为列表形式
            messages = [self.tokenizer.apply_chat_template(sentence, tokenize=False) for sentence in messages]
            # 应用聊天模板到每个消息上
