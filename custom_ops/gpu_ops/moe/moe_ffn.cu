...
void MoeFFNKernel(const paddle::Tensor &permute_input,
  ...
  if (quant_method == "weight_only_int8") {
    int8_moe_gemm_runner.moe_gemm_bias_act(
        ...
        total_rows_in_ll_else_minus1, tune_total_rows, inter_size, hidden_size,
        num_experts, "none", stream);
    ...
    w4a8_moe_gemm_runner.moe_gemm(
        ...
        reinterpret_cast<char *>(workspace->ptr()), workspace_size, num_experts,
        stream);
  } else {
    fp16_moe_gemm_runner.moe_gemm_bias_act(
        reinterpret_cast<const NvType *>(permute_input.data<data_t>()),
        reinterpret_cast<const NvType *>(ffn1_weight.data<data_t>()), nullptr,
        reinterpret_cast<const NvType *>(fc1_expert_biases),
        reinterpret_cast<NvType *>(fc1_out),
        const_cast<int64_t *>(tokens_expert_prefix_sum.data<int64_t>()),
        total_rows_in_ll_else_minus1, tune_total_rows, inter_size, hidden_size,
  ...
  if (quant_method == "weight_only_int8") {
    int8_moe_gemm_runner.moe_gemm(
        ...
        const_cast<int64_t *>(tokens_expert_prefix_sum.data<int64_t>()),
        total_rows_in_ll_else_minus1, tune_total_rows, hidden_size,
        inter_size / 2, num_experts, stream);
    ...
    MoeFastHardamardWrapper<data_t, int8_t>(
        ...
        ffn2_in_scale ? const_cast<paddle::Tensor *>(ffn2_in_scale.get_ptr())
                            ->data<float>()
                      : nullptr,
        1, 127.0, -127.0, expanded_active_expert_rows, inter_size / 2,
        reinterpret_cast<int8_t *>(int8_act_out->ptr()), stream);
    w4a8_moe_gemm_runner.moe_gemm(
    ...
    fp16_moe_gemm_runner.moe_gemm(
        reinterpret_cast<const NvType *>(act_out),
        reinterpret_cast<const NvType *>(ffn2_weight.data<data_t>()), nullptr,
        reinterpret_cast<NvType *>(ffn_out_data),
        const_cast<int64_t *>(tokens_expert_prefix_sum.data<int64_t>()),
        total_rows_in_ll_else_minus1, tune_total_rows, hidden_size,
        inter_size / 2, num_experts, stream);
...
std::vector<paddle::Tensor> MoeExpertFFN(
    ...
    const paddle::optional<paddle::Tensor> &expert_idx_per_token,
    const std::string &quant_method, const bool used_in_ep_low_latency) {
  return {MoeExpertFFNFunc(permute_input, tokens_expert_prefix_sum, ffn1_weight,
                           ffn2_weight, ffn1_bias, ffn1_scale, ffn2_scale,
                           ffn2_in_scale, expert_idx_per_token, quant_method,
...
PD_BUILD_STATIC_OP(moe_expert_ffn)
    .Inputs({"permute_input", "tokens_expert_prefix_sum", "ffn1_weight",
             "ffn2_weight", paddle::Optional("ffn1_bias"),
             paddle::Optional("ffn1_scale"), paddle::Optional("ffn2_scale"),
