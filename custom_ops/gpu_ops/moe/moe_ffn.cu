...
void WeightOnlyMoeFFNKernel(const paddle::Tensor& permute_input,
    ...
    using namespace phi;
    using WeightOnlyTraits = WintQuantTraits<NvType, QuantMethod>;
    ...
    moe_gemm_runner.moe_gemm_bias_act(
        ...
        reinterpret_cast<const NvType*>(ffn1_scale ? ffn1_scale->data<DataT>() : nullptr),
    ...
    moe_gemm_runner.moe_gemm(
        ...
        reinterpret_cast<const NvType*>(ffn2_scale ? ffn2_scale->data<DataT>() : nullptr),
...
void W4A8MoeFFNKernel(const paddle::Tensor& permute_input,
    ...
    if (used_in_ep_low_latency) {
        PD_THROW("used_in_ep_low_latency = true is disable temporarly!");
        // act_out_tensor = GroupSwigluWithMasked(fc1_out, tokens_expert_prefix_sum);
    } else {
        act_out = paddle::experimental::swiglu(fc1_out, nullptr);
    }
...
void MoeFFNKernel(const paddle::Tensor& permute_input,
    ...
    if (quant_method == "weight_only_int8") {
        WeightOnlyMoeFFNKernel<data_t, NvType, int8_t, WintQuantMethod::kWeightOnlyInt8>(
            permute_input,
        ...
        WeightOnlyMoeFFNKernel<data_t, NvType, int8_t, WintQuantMethod::kWeightOnlyInt4>(
            permute_input,
            tokens_expert_prefix_sum,
            ffn1_weight,
            ffn2_weight,
...
*/
        ...
        WeightOnlyMoeFFNKernel<data_t, NvType, data_t, WintQuantMethod::kNone>(
            permute_input,
            tokens_expert_prefix_sum,
            ffn1_weight,
            ffn2_weight,
